...
$email = $password = $type = $username_w = "";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $username_w, $hashed_password, $email, $type);
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($password, $hashed_password)){
                            ...
                            $_SESSION["id"] = $id;
                            $_SESSION["username_w"] = $username_w;
    ...
    <main class="page contact-page">
        <section class="portfolio-block contact" style="padding-top: 50px;padding-bottom: 50px;">
            <div class="container">
                ...
                <form style="padding-top: 40px;padding-left: 40px;" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    ...
                    <div class="form-group <?php echo (!empty($password_err)) ? 'has-error' : ''; ?>">
                        ...
                        <input class="form-control item" type="password" id="password" name="password" value="<?php echo $password; ?>">
                        <span class="help-block"><?php echo $password_err; ?></span>
                    </div>
                    <div class="form-group"><button class="btn btn-primary btn-block btn-lg" type="submit" value="Login">Log-in</button></div>
                </form>
            </div>
        </section>
    </main>
