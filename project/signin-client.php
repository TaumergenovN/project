            ...
            <div class="container">
                ...
                <form style="margin-top: 0px;padding-top: 40px;padding-left: 40px;" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                    <div class="form-group <?php echo (!empty($username_err)) ? 'has-error' : ''; ?>">
                        ...
                        <input autocomplete="off" class="form-control item" type="text" id="name" name="username" value="<?php echo $username; ?>">
                        <span class="help-block"><?php echo $username_err; ?></span>
                    </div>
                    <div class="form-group <?php echo (!empty($email_err)) ? 'has-error' : ''; ?>">
                        <label for="email">Email</label>
                        <input autocomplete="off" class="form-control item" type="email" id="email" name="email" value="<?php echo $email; ?>">
                        <span class="help-block"><?php echo $email_err; ?></span>
                    </div>
                    <div class="form-group <?php echo (!empty($password_err)) ? 'has-error' : ''; ?>">
                        <label for="subject">Password</label>
                        <input autocomplete="off" class="form-control item" type="password" id="subject" name="password" value="<?php echo $password; ?>">
                        <span class="help-block"><?php echo $password_err; ?></span>
                    </div>
                    <div class="form-group <?php echo (!empty($confirm_password_err)) ? 'has-error' : ''; ?>">
                        <label for="subject">Confirmation of password</label>
                        <input autocomplete="off" class="form-control item" type="password" id="subject-1" name="confirm_password" value="<?php echo $confirm_password; ?>">
                        <span class="help-block"><?php echo $confirm_password_err; ?></span>
                    </div>

                    <div class="form-group"><button class="btn btn-primary btn-block btn-lg" value="Submit" type="submit">Sign-in</button></div>
                </form>
            </div>
        </section>
    </main>
